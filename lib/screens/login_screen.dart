...
class _LoginScreenState extends State<LoginScreen>
  ...
  bool _isPasswordStrong(String password) {
    ...
    // Check for at least one special character (optional for medium)
    bool hasSpecialChar = password.contains(RegExp(r'[!@#$%^&*(),.?":{}|<>]'));
    
    // Medium password: length >= 8, uppercase, lowercase, number
    bool isMedium = hasUppercase && hasLowercase && hasNumber;
    
    // Strong password: all above + special character
    bool isStrong = isMedium && hasSpecialChar;
    
    return isMedium || isStrong;
  }
  ...
  Future<void> _handleLogin() async {
    ...
    if (mounted) {
      ...
      // Check if it's a valid email format and has medium/strong password
      bool isValidEmail = RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(email);
      bool isValidPassword = _isPasswordStrong(password);
      ...
      if (mounted && Navigator.canPop(context)) {
        ...
        Timer(const Duration(milliseconds: 100), () async {
          if (mounted) {
            ...
            final email = _emailController.text.trim().toLowerCase();
            final name = AuthService.extractNameFromEmail(email);
            await AuthService.saveUserLogin(email: email, name: name);
                                        ...
                                        child: Container(
                                          ...
                                          child: Column(
                                            children: [
                                              ...
                                              TextFormField(
                                                ...
                                                decoration: InputDecoration(
                                                  ...
                                                  prefixIcon: AnimatedBuilder(
                                                    ...
                                                    builder: (context, child) {
                                                      return Transform.rotate(
                                                        angle: _iconRotateAnimation.value * 0.5,
                                                        child: Icon(
                                                          Icons.lock,
                                                          color: Color.lerp(
                                                            const Color(0xFFFFD700),
                                                  ...
                                                  suffixIcon: AnimatedBuilder(
                                                    ...
                                                    builder: (context, child) {
                                                      return Transform.scale(
                                                        scale: 1.0 + (_pulseAnimation.value - 1).abs() * 0.2,
                                                        child: IconButton(
                                                          icon: Icon(
                                              ...
                                              // Forgot password link (above Sign In button)
                                              Align(
